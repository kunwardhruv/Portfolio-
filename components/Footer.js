...
const SOCIALS = [
  {
    label: 'GitHub', href: 'https://github.com/kunwardhruv',
    svg: <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M12 0C5.37 0 0 5.37 0 12c0 5.31 3.435 9.795 8.205 11.385.6.105.825-.255.825-.57 0-.285-.015-1.23-.015-2.235-3.015.555-3.795-.735-4.035-1.41-.135-.345-.72-1.41-1.23-1.695-.42-.225-1.02-.78-.015-.795.945-.015 1.62.87 1.845 1.23 1.08 1.815 2.805 1.305 3.495.99.105-.78.42-1.305.765-1.605-2.67-.3-5.46-1.335-5.46-5.925 0-1.305.465-2.385 1.23-3.225-.12-.3-.54-1.53.12-3.18 0 0 1.005-.315 3.3 1.23.96-.27 1.98-.405 3-.405s2.04.135 3 .405c2.295-1.56 3.3-1.23 3.3-1.23.66 1.65.24 2.88.12 3.18.765.84 1.23 1.905 1.23 3.225 0 4.605-2.805 5.625-5.475 5.925.435.375.81 1.095.81 2.22 0 1.605-.015 2.895-.015 3.3 0 .315.225.69.825.57A12.02 12.02 0 0 0 24 12c0-6.63-5.37-12-12-12z"/></svg>
  },
  {
    label: 'X / Twitter', href: 'https://x.com/tomardhruvsingh',
  ...
  {
    label: 'LinkedIn', href: 'https://www.linkedin.com/in/dhruv-singh-24nov2004',
  ...
  {
    label: 'Instagram', href: 'https://www.instagram.com/kunwar.dhruvv/',
  ...
  {
    label: 'Email', href: 'mailto:kunwarrdhruv@gmail.com',
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1.5rem', marginBottom: '2rem' }}>
        ...
        <div>
          <div style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: '1.75rem', color: '#f59e0b', letterSpacing: '-0.02em', marginBottom: '0.5rem' }}>
            DS<span style={{ color: 'var(--text)' }}>.</span>
          </div>
          <p style={{ fontFamily: 'var(--font-mono)', fontSize: '0.8rem', letterSpacing: '0.15em', color: 'var(--text-sub)', textTransform: 'uppercase' }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
          {SOCIALS.map(s => (
            <a key={s.label} href={s.href} target={s.href.startsWith('mailto') ? undefined : '_blank'} rel="noopener noreferrer" title={s.label}
              ...
              onMouseEnter={e => { e.currentTarget.style.borderColor = '#f59e0b'; e.currentTarget.style.color = '#f59e0b'; e.currentTarget.style.transform = 'translateY(-2px)' }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.color = 'var(--text-sub)'; e.currentTarget.style.transform = 'none' }}>
              {s.svg}
            </a>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', gap: '1.5rem' }}>
          {NAV.map(n => (
            <Link key={n.label} href={n.href}
              style={{ fontFamily: 'var(--font-mono)', fontSize: '0.78rem', color: 'var(--text-sub)', textDecoration: 'none', letterSpacing: '0.1em', transition: 'color 0.2s' }}
